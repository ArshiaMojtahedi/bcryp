...
def main():
    ...
    while True:
        ...
        print('programa ja rodou ',count_time_random)
        count_time_random+=1
        
        if count_time_random == randint(90,100):
            logger.logger('programa descansando e zerando')
            time.sleep(randint(2,5))
            count_time_random = 0
        
        now = time.time()
        
        action = randint(0,len(random_action))
        logger.logger("It's time to select Action number " + str(action))
